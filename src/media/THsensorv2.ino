...
byte mac[] = { 0xDE, 0xAD, 0xBE, 0x41, 0x42, 0x22 }; // make sure you change these values so that no MAC collision exist in your network

uint8_t IPaddr[4];
uint8_t DeviceCode;
const String DeviceType="THsensorv2"; 
byte STATUS_bits;
float T_centdeg;
float RH_percent;
...
//  const char _BLANKLINE_ ;
  ...
  struct XML_TAGS_FLOAT FLOAT_DATA_TAGS[3];
  const char _cXML_TAG_[5];
}xml_response_data2 = {
...
//                 '\n',
                 ...
                 {'<','D','I','d','>'},{0},{'<','/','D','I','d','>'},
                 {var_tag},{var_tag_float,var_tag_float,var_tag_float},
                 {'<','/','X','>','\n'}  
                };

const char *pXML_RESP_data2 = &xml_response_data2._XML_HEADER_[0];
const int __SIZE_XML_RESPONSE_data2__ = sizeof(xml_response_data2);
...
void responseREQ(byte *baseREQ,byte *orderREQ,byte *webREQ)
{
    ...
    case 2:   // request for "data2.xml"
      ...
      for (byte i=0;i<4;i++)
      {
         ...
         var[i+9]=*((byte*)&DewPoint_centdeg+3-i);

      }
      XML_response2(&var[0],__SIZE_XML_RESPONSE_data2__,*pXML_RESP_data2);
      delay(10);
      *baseREQ=0;
      break;        
